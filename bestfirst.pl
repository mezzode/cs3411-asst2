% Bratko Figure 12.3. A best-first search program.

% bestfirst(Start, Solution): Solution is a path from Start to a goal state
% Search tree represented as a term l(Node, Score) for leaf nodes and a term
% t(Node, Cost, TreeList) for a complex tree

solve(Start, Solution) :-
	expand([], l(Start, 0/0),  9999, _, yes, Solution).	%  Assume 9999 is greater than any f-value

% expand(Path, Tree, Bound, Tree1, Solved, Solution)
% Path is path between start node of search and subtree Tree in reverse order
% Tree1 is Tree expanded within Bound
% if goal found then Solution is solution path and Solved = yes

% Case 1: goal leaf-node, return a solution path
...
expand(P, l(N, F/G), Bound, Tree1, Solved, Sol) :-
	F =< Bound,
	(findall(M/C, (s(N, M, C), not(member(M, P))), Succ), not(Succ = []),
...
% Insert subtree in order of f-value and continue search from best subtree
% or if no possible solution, discard this subtree and continue from next best

continue(_, _, _, _, yes, yes, Sol).

continue(P, t(N, F/G, [T1|Ts]), Bound, Tree1, no, Solved, Sol) :-
	insert(T1, Ts, NTs),			% insert tree in order of f-value
	bestf(NTs, F1),
...
% Make list of leaf nodes ordered by f-value
